...
public class AchievementController : MonoBehaviour
{
    ...
    public AudioSource soundAchiev;
    ...
    {
        // Mencari data achievement yang sudah diinput
        AchievementData achievement = _achievementList.Find(a => a.Type == type && a.Value == value);
        if (achievement != null && !achievement.IsUnlocked)
        {
            soundAchiev.PlayOneShot(soundAchiev.clip);
            achievement.IsUnlocked = true;
            ShowAchivementPopUp(achievement);
        }
    }

    //Menampilkan achievement yang dicapai sesuai achievement yang tersedia
